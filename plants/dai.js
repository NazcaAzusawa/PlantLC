...
const daiDifficulty = {
    ...
    damageScale: 'Anthes'    // 予想被害規模: Anthes（仮）
};

// ダイダイダイの管理方法データ
const daiHintContents = {
    'dai-1': { text: 'ダイダイダイは特別な管理が必要です。' }
};

const daiHints = [
    { id: 'dai-1', name: 'ダイダイダイの管理方法1', price: 5 }
];

// ダイダイダイのエッセンス計算関数
function calculateEssenceDai(plant) {
    const params = plant.harvestParams;
    const { waterCount, fertilizerColor, fertilizerGiven, lightLevel, tempLevel } = params;
    
    console.log('ダイダイダイ エッセンス計算:', params);
    ...
    console.log('ダイダイダイ 計算結果:', { totalScore, essence });
...
if (typeof module !== 'undefined') {
    module.exports = { calculateEssenceDai };
}
